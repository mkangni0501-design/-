...
export type ReportCardStyleConfig = {
  colors: {
    ...
    // 【本輪新增】反映事項「所有上學期的內容用一個顏色,下學期的內容用一個顏色」：
    // 內頁凡是「上學期」欄位的內容格（科目分數、出缺席/懲獎次數、全班人數/名次、
    // 操行成績等）統一用 springTermBg，「下學期」統一用 fallTermBg，兩種顏色跟
    // 現有 cellLeftLabelBg（淡黃，列標籤用）、sectionTitleBg（灰，標題用）區分開。
    springTermBg: string;
    fallTermBg: string;
    borderColor: string;
  };
...
function buildStyles(config: ReportCardStyleConfig) {
  ...
  return {
    ...
    sheet: StyleSheet.create({
      ...
      // 【本輪新增】上/下學期內容格專用底色（見上面 colors.springTermBg／fallTermBg
      // 的說明），沿用 cell 其餘設定（置中、邊框），只是多疊一個 backgroundColor。
      springCell: { textAlign: 'center', fontSize: base, padding: 3, justifyContent: 'center', borderLeft: BORDER, backgroundColor: config.colors.springTermBg },
      fallCell: { textAlign: 'center', fontSize: base, padding: 3, justifyContent: 'center', borderLeft: BORDER, backgroundColor: config.colors.fallTermBg },
...
// 封面頁校徽尺寸：跟「操行成績標準」標題+5行文字的實際渲染高度對齊（用
// pdftoppm 實測量過，約98pt），見下面校徽 View 的說明。
const SCHOOL_LOGO_SIZE = 98;
  ...
  return (
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.row}>
        ...
        <View style={[styles.cellHead, { width: `${scoreColWidth * 4}%`, backgroundColor: config.colors.springTermBg }]}>
          <Text>上學期</Text>
        </View>
        <View style={[styles.cellHead, { width: `${scoreColWidth * 4}%`, backgroundColor: config.colors.fallTermBg }]}>
          <Text>下學期</Text>
        </View>
      ...
      <View style={styles.row}>
        ...
        {(['期中', '期末', '平時', '總分'] as const).map((h) => (
          <View key={'s' + h} style={[styles.cellHead, styles.scoreCol, { backgroundColor: config.colors.springTermBg }]}>
            <Text>{h}</Text>
          </View>
        ))}
        {(['期中', '期末', '平時', '總分'] as const).map((h) => (
          <View key={'f' + h} style={[styles.cellHead, styles.scoreCol, { backgroundColor: config.colors.fallTermBg }]}>
            <Text>{h}</Text>
          </View>
        ))}
        ...
        return (
          <View style={[styles.row, { minHeight: 12 }]}>
            ...
            {[sw.midterm, sw.final, sw.daily, sw.midterm + sw.final + sw.daily].map((v, i) => (
              <View key={'sw-s' + i} style={[styles.cell, styles.scoreCol, { minHeight: 12, padding: 1, backgroundColor: config.colors.springTermBg }]}>
                <Text style={{ fontSize: 6.5 }}>{pctText(v)}</Text>
              </View>
            ))}
            {[sw.midterm, sw.final, sw.daily, sw.midterm + sw.final + sw.daily].map((v, i) => (
              <View key={'sw-f' + i} style={[styles.cell, styles.scoreCol, { minHeight: 12, padding: 1, backgroundColor: config.colors.fallTermBg }]}>
                <Text style={{ fontSize: 6.5 }}>{pctText(v)}</Text>
              </View>
            ))}
      ...
      {subjectRows.map((s, idx) => {
        ...
        return (
          <View style={[styles.row, { flex: 1, minHeight: 0, overflow: 'hidden' }]} key={s?.subject ?? `blank-${idx}`}>
            ...
            <View style={[styles.cell, styles.scoreCol, { fontSize: denseFontSize, padding: densePadding, backgroundColor: config.colors.springTermBg }]}>
            ...
            <View style={[styles.cell, styles.scoreCol, { fontSize: denseFontSize, padding: densePadding, backgroundColor: config.colors.fallTermBg }]}>
      ...
      <View style={[styles.row, { flex: 1, minHeight: 0, overflow: 'hidden' }]}>
        ...
        {(['midterm', 'final', 'daily', 'total'] as const).map((k) => (
          <View key={'sa' + k} style={[styles.cell, styles.scoreCol, { fontSize: denseFontSize, padding: densePadding, backgroundColor: config.colors.springTermBg }]}>
            <Text>{fmtRounded(terms.上學期?.academicAverage[k])}</Text>
          </View>
        ))}
        {(['midterm', 'final', 'daily', 'total'] as const).map((k) => (
          <View key={'fa' + k} style={[styles.cell, styles.scoreCol, { fontSize: denseFontSize, padding: densePadding, backgroundColor: config.colors.fallTermBg }]}>
      ...
      <View style={{ flexDirection: 'row', borderBottom: BORDER, flex: 5, minHeight: 0, overflow: 'hidden' }}>
        ...
        {/* 上學期：分數(2欄寬) + 等第(2欄寬，貫穿五列)。
            ...
            flex:3/flex:1。 */}
        <View style={{ width: `${scoreColWidth * 4}%`, flexDirection: 'row', borderLeft: BORDER, backgroundColor: config.colors.springTermBg }}>
        ...
        <View style={{ width: `${scoreColWidth * 4}%`, flexDirection: 'row', borderLeft: BORDER, backgroundColor: config.colors.fallTermBg }}>
          <View style={{ flex: 2, flexDirection: 'column' }}>
            {(['overall', 'politeness', 'dress', 'service', 'discipline'] as const).map((key, i) => (
              <View key={key} style={{ flex: 1, minHeight: 0, overflow: 'hidden', justifyContent: 'center', alignItems: 'center', borderBottom: i < 4 ? BORDER : 'none' }}>
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={{ height: LEFT_HEADER_FIXED_PT }}>
        ...
        <View style={[styles.row, { flex: 1 }]}>
          ...
          <View style={[styles.cellHead, styles.attnValCol, { backgroundColor: config.colors.springTermBg }]}>
            <Text>上學期</Text>
          </View>
          <View style={[styles.cellHead, styles.attnValCol, { backgroundColor: config.colors.fallTermBg }]}>
            <Text>下學期</Text>
          </View>
      ...
      <View style={{ flex: 1 }}>
          ...
          {[0, 1, 2, 3, 4, 5].map((i) => {
            ...
            return (
              <View style={[styles.row, { flex: 1, minHeight: 0 }]} key={i}>
                ...
                <View style={[styles.cell, styles.attnValCol, { backgroundColor: config.colors.springTermBg }]}>
                  <Text style={{ fontSize: styles.cell.fontSize }}>
                    {isPerfectRow ? (spring ? (spring.isPerfectAttendance ? '是' : '否') : '') : attnKey ? springV ?? '' : ''}
                  </Text>
                </View>
                <View style={[styles.cell, styles.attnValCol, { backgroundColor: config.colors.fallTermBg }]}>
                  <Text style={{ fontSize: styles.cell.fontSize }}>
        ...
        {/* 簽章列：從左表「服務」列上緣開始（前面補 flex:gapFlex 的空白間隔），
            ...
            body 內部本來就已經精準對齊的比例。 */}
        {/* 【2026-09-04 修正】反映事項「升留級、家長簽章及建議下方表格也加入垂直
    ...
    <Page size="A4" orientation="landscape" style={{ padding: 20, fontFamily: 'NotoSansTC', fontSize: 9 }}>
      ...
      <View style={{ flexDirection: 'row', flex: 1 }}>
        ...
        <View style={{ flex: 1, paddingLeft: 10 }}>
          {/* 【2026-09-04 修正】反映事項「校徽放大到【不滿60分者為丁等(不及格)】的
              高度」：原本校徽固定 60×60pt，比中折頁「操行成績標準」那五行文字
              （含標題）矮很多。實際用 pdftoppm 渲染量測過「操行成績標準：」標題
              第一行頂端到最後一行「不滿60分者為丁等（不及格）」底部的高度，
              換算成 pt 大約 98pt，這裡校徽（含沒有圖檔時的色塊佔位）改成
              SCHOOL_LOGO_SIZE（98×98pt）跟這個高度對齊，不再是原本的 60×60。 */}
          <View style={{ height: '18%', alignItems: 'center', justifyContent: 'flex-start' }}>
            {logoSrc ? (
              <Image src={logoSrc} style={{ width: SCHOOL_LOGO_SIZE, height: SCHOOL_LOGO_SIZE }} />
            ) : (
              ...
              <View
                style={{
                  width: SCHOOL_LOGO_SIZE,
                  height: SCHOOL_LOGO_SIZE,
                  borderRadius: SCHOOL_LOGO_SIZE / 2,
          ...
          <View style={styles.outer}>
            ...
            <View style={styles.body}>
              ...
              <View style={styles.rightCol}>
                <AttendanceDisciplinePanel terms={data.terms} styles={styles} BORDER={BORDER} labels={labels} config={config} />
              </View>
            </View>
