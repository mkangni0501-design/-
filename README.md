...
app/
  ...
  (app)/admin/exam-seating/page.tsx -- 【考試分班】教務處：新增/刪除考試 → 可勾選「不擔任考場班級」（自動排除，該班學生仍可被安排到其他班考場）→ 系統自動把其餘班級設為考場並持續同步座位數＝該班目前在校人數（未確認的考場人數異動會自動校正，已確認的只會警示不會硬改）→ 考場旁邊直接展開勾選應試班級群組（最多4班，考場自己的班級一定包含，選了誰對方考場也會自動連動勾選回來，全部改完只要按一次【儲存應試班級】）→ 依分組（只顯示有共用考場的班級群組，簡化表格）試算各考場人數，兩項驗證不合時可按【自動調整】自動搬動名額（仍保留逐格手動調整）、【儲存分配人數】改成單次批次寫入不再逐列等待 → 可一次瀏覽所有考場的梅花座位表（點兩格互換做手動微調，座位上一律會顯示班級＋姓名／座號）後一鍵確認全部 → 全部考場完成後發送考場表通知各班導師 → 發送後可預覽各考場名單/座位（含班級、姓名、座號、學號）並列印座位表＋簽到表（簽到表依班級、原班座號排序，合併在一張A4）、也可以一鍵下載全部考場的簽到表／座位表 Excel（每間考場各兩個分頁）
  (app)/exam-rosters/page.tsx -- 【考試分班】導師：收到考場通知後，把本班學生填入分配到的考場座位（可隨機分配），完成後送出並鎖定；管理員A、系統管理員S、教務處另外可以直接代替任何一班的導師安排（不限自己帶的班），選考試後可用批次勾選清單（含全選未送出班級）一鍵幫多個班級隨機安排座位並直接送出鎖定（人數比座位多、還沒排滿的班級不會自動送出，會列在提示訊息裡請手動確認），讓考試分班頁能列印座位表／簽到表；也可以點班級名稱切換到該班詳細畫面逐一調整
