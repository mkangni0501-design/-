...
app/
  ...
  (app)/admin/exam-seating/page.tsx -- 【考試分班】教務處：新增/刪除考試 → 系統自動把所有班級設為考場（考場名稱＝班級名稱、座位數＝該班目前在校人數，不用另外手動新增）→ 考場旁邊直接展開選擇應試班級（最多4班）並儲存 → 依座位數比例試算各考場人數（雙驗證，可手動修改）→ 產生梅花座位表並確認 → 全部考場完成後發送考場表通知各班導師 → 發送後可預覽各考場名單/座位並列印座位表、簽到表
...
lib/
  ...
  examSeating.ts -- 【考試分班】考試/考場/應試班級 CRUD、依座位數比例計算各考場人數（含最大餘數法校正＋雙驗證）、梅花座位表演算法（方形座位、最大7*7、公平交錯排班讓同班盡量不相鄰）
public/
